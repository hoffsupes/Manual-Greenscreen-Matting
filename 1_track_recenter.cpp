...
void get_newIa(Mat I, Mat alpha, Mat &In, Mat &alphan)
{
    resize(I,I,I.size()/4);
    resize(alpha,alpha,alpha.size()/4);
    
    I.copyTo( In( Rect(800,800,I.cols,I.rows) ) );
    alpha.copyTo( alphan( Rect(800,800,I.cols,I.rows) ) );
}
...
void track_1( int, void* opn )
{
    ...
    Mat In,alphan;
    In = Mat::zeros(T.size(),CV_32F);
    alphan = Mat::zeros(T.size(),CV_32F);
    
    get_newIa(I,alpha,In,alphan);
    
    Mat Y =  apply_alpha(T, 1-alphan) + apply_alpha(In, alphan);       // apply alpha, multiply alpha to each channel of the given image
